...
FILE_NAME="mbworkerbot"
MODULE="{{ mbworkerbot.dirs.module }}"
BILL_HOME="{{ billing.dirs.home }}"
VERSION="{{ mbworkerbot.dirs.production }}"
WORK_DIR="${BILL_HOME}/${MODULE}/${VERSION}"
UPD_DIR="${BILL_HOME}/${MODULE}/${VERSION}/{{ mbworkerbot.dirs.updates }}"
BKP_DIR="${BILL_HOME}/${MODULE}/${VERSION}/{{ billing.dirs.backups }}"
TMP_DIR="{{ billing.dirs.tmp }}"

UPDATER_REVISION=1
UPDATER_NAME="{{ mbworkerbot.files.updates }}"

DATENAME=$(date +%F_%H-%M)
INFO="[${DATENAME} ${VERSION}]"
LOG_DIR="${BILL_HOME}/{{ billing.dirs.logs }}/${MODULE}"
FILE_LOG="${LOG_DIR}/{{ mbworkerbot.files.log_update }}"
...
if [ "${MD5_CURRENT}" != "${MD5_DOWNLOADED}" ]; then
  ...
  if [ -f "./mbworkerbot.php" ]; then
    if [ ! -d "${BKP_DIR}" ]; then
      mkdir -p "${BKP_DIR}"
    fi

    upd_log "backup current version to ${BKP_DIR}/${FILE_NAME}_${VERSION}.${DATENAME}.tar.gz"
    tar -zcf "${BKP_DIR}/${FILE_NAME}_${VERSION}.${DATENAME}.tar.gz" "mbworkerbot.php"
  fi
