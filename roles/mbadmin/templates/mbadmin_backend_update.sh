...
FILE_NAME="mbadmin"
MODULE="{{ mbadmin.dirs.module }}"
BILL_HOME="{{ billing.dirs.home }}"
VERSION="{{ mbadmin.dirs.production }}"
WORK_DIR="${BILL_HOME}/${MODULE}/${VERSION}/public"
UPD_DIR="${BILL_HOME}/${MODULE}/${VERSION}/{{ mbadmin.dirs.updates }}"
BKP_DIR="${BILL_HOME}/${MODULE}/${VERSION}/{{ billing.dirs.backups }}"
TMP_DIR="{{ billing.dirs.tmp }}"

UPDATER_REVISION=1
UPDATER_NAME="{{ mbadmin.files.update_backend }}"

DATENAME=$(date +%F_%H-%M)
INFO="[${DATENAME} ${VERSION}]"
LOG_DIR="${BILL_HOME}/{{ billing.dirs.logs }}/${MODULE}"
FILE_LOG="${LOG_DIR}/{{ mbadmin.files.log_update }}"
...
if [ "${MD5_CURRENT}" != "${MD5_DOWNLOADED}" ]; then
  ...
  if [ -f "./index.php" ]; then
    ...
    tar -zcf "${BKP_DIR}/${FILE_NAME}_${VERSION}.${DATENAME}.tar.gz" "index.php"
  fi
  ...
  if [ -f "${BILL_HOME}/mbdbupdate/production/mbdbupdate_update.sh" ]; then
    upd_log "apply sql update"
    bash "${BILL_HOME}/mbdbupdate/production/mbdbupdate_update.sh"
  fi
else
