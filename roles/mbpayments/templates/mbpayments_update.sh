...
FILE_NAME="mbpayments"
MODULE="{{ mbpayments.dirs.module }}"
BILL_HOME="{{ billing.dirs.home }}"
VERSION="{{ mbpayments.dirs.production }}"
WORK_DIR="${BILL_HOME}/${MODULE}/${VERSION}/public"
UPD_DIR="${BILL_HOME}/${MODULE}/${VERSION}/{{ mbpayments.dirs.updates }}"
BKP_DIR="${BILL_HOME}/${MODULE}/${VERSION}/{{ billing.dirs.backups }}"
TMP_DIR="{{ billing.dirs.tmp }}"

UPDATER_REVISION=1
UPDATER_NAME="{{ mbpayments.files.update_backend }}"

DATENAME=$(date +%F_%H-%M)
INFO="[${DATENAME} ${VERSION}]"
LOG_DIR="${BILL_HOME}/{{ billing.dirs.logs }}/${MODULE}"
FILE_LOG="${LOG_DIR}/{{ mbpayments.files.log_update }}"
